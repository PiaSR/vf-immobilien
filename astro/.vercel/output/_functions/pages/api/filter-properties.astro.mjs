...
const buildGroqFilter = (formData) => {
  const filters = [];
  ...
  if (zimmerMin && zimmerMin.length > 0 && !isNaN(Number(zimmerMin))) {
    filters.push(`rooms >= ${zimmerMin}`);
  }
  if (zimmerMax && zimmerMax.length > 0 && !isNaN(Number(zimmerMax))) {
    filters.push(`rooms <= ${zimmerMax}`);
  }
  const flaecheMin = params.get("flaeche-min");
  const flaecheMax = params.get("flaeche-max");
  if (flaecheMin && flaecheMin.length > 0 && !isNaN(Number(flaecheMin))) {
    filters.push(`livingArea >= ${flaecheMin}`);
  }
  if (flaecheMax && flaecheMax.length > 0 && !isNaN(Number(flaecheMax))) {
    filters.push(`livingArea <= ${flaecheMax}`);
  }
