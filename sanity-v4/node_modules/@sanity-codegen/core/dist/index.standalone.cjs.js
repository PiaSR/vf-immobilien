...
const tsTypeForCircularRefs = node => t__namespace.tsTypeReference(t__namespace.tsQualifiedName(t__namespace.tsQualifiedName(t__namespace.identifier('Sanity'), t__namespace.identifier('Ref')), t__namespace.identifier(`Ref_${node.hash}`)));
...
function transformStructureToTs({
  ...
  return {
    ...
    substitutions: Object.fromEntries(Array.from(aliasTypes).map(([hash]) => [hash, t__namespace.tsTypeReference(t__namespace.tsQualifiedName(t__namespace.tsQualifiedName(t__namespace.identifier('Sanity'), t__namespace.identifier('Ref')), t__namespace.identifier(`Ref_${hash}`)))])),
    declarations: Object.fromEntries(Array.from(aliasTypes).map(([hash, tsType]) => [hash, t__namespace.tsModuleDeclaration(t__namespace.identifier('Sanity'), t__namespace.tsModuleDeclaration(t__namespace.identifier('Ref'), t__namespace.tsModuleBlock([t__namespace.tsTypeAliasDeclaration(t__namespace.identifier(`Ref_${hash}`), undefined, tsType)])))]))
  };
}
...
function transform({
  ...
  switch (node.type) {
    case 'And':
      {
        tsType = t__namespace.tsIntersectionType(node.children.sort((a, b) => a.hash.localeCompare(b.hash, 'en')).map(next));
        break;
      }
    case 'Boolean':
      {
    ...
    case 'Object':
      {
        tsType = t__namespace.tsTypeLiteral(node.properties.sort((a, b) => a.key.localeCompare(b.key, 'en')).map(({
          key,
          value
        }) => {
          const valueIsOptional = isStructureOptional(value);
          const propertySignature = t__namespace.tsPropertySignature(t__namespace.stringLiteral(key), t__namespace.tsTypeAnnotation(next(valueIsOptional ? removeOptional(value) : value)));
    ...
    case 'Or':
      {
        tsType = t__namespace.tsUnionType(node.children.sort((a, b) => a.hash.localeCompare(b.hash, 'en')).map(next));
        break;
      }
    case 'String':
      {
        tsType = node.value ? t__namespace.tsLiteralType(t__namespace.stringLiteral(node.value)) : t__namespace.tsStringKeyword();
        break;
      }
    case 'Unknown':
      {
        tsType = t__namespace.tsUnknownKeyword();
        break;
      }
    case 'Lazy':
      {
    ...
    case 'Reference':
      {
        tsType = t__namespace.tsTypeReference(t__namespace.tsQualifiedName(t__namespace.identifier('Sanity'), t__namespace.identifier('Reference')), t__namespace.tsTypeParameterInstantiation([next(node.to)]));
        break;
      }
    case 'Array':
      {
        tsType = t__namespace.tsArrayType(next(isStructureOptional(node.of) ? removeOptional(node.of) : node.of));
        break;
      }
    case 'Tuple':
      {
        tsType = t__namespace.tsTupleType(node.elements.map(next));
        break;
      }
    default:
      {
  ...
  if ('canBeNull' in node || 'canBeOptional' in node) {
    const types = [tsType];
    if (node.canBeNull) types.push(t__namespace.tsNullKeyword());
    if (node.canBeOptional) types.push(t__namespace.tsUndefinedKeyword());
    tsType = t__namespace.tsUnionType(types);
  }
  return tsType;
}
...
function generateQueryTypes({
  ...
  const queries = extractedQueries.map(({
    ...
    return {
      ...
      declarations: {
        ...declarations,
        [structure.hash]: t__namespace.tsModuleDeclaration(t__namespace.identifier('Sanity'), t__namespace.tsModuleDeclaration(t__namespace.identifier(workspaceIdentifier), t__namespace.tsModuleDeclaration(t__namespace.identifier('Query'), t__namespace.tsModuleBlock([t__namespace.tsTypeAliasDeclaration(t__namespace.identifier(queryKey), undefined, tsType)]))))
      },
      substitutions: {
        ...substitutions,
        [structure.hash]: t__namespace.tsTypeReference(t__namespace.tsQualifiedName(t__namespace.tsQualifiedName(t__namespace.tsQualifiedName(t__namespace.identifier('Sanity'), t__namespace.identifier(workspaceIdentifier)), t__namespace.identifier('Query')), t__namespace.identifier(queryKey)))
      }
    };
  }).filter(t => !!t);
  const substitutions = queries.reduce((acc, {
    ...
    for (const [hash, substitution] of Object.entries(substitutions)) {
      if (!acc[hash]) {
        ...
        acc[`${hash}_${queryKey}`] = t__namespace.tsTypeReference(t__namespace.tsQualifiedName(t__namespace.tsQualifiedName(t__namespace.tsQualifiedName(t__namespace.identifier('Sanity'), t__namespace.identifier(workspaceIdentifier)), t__namespace.identifier('Query')), t__namespace.identifier(queryKey)));
      }
    }
    return acc;
  }, {});
  const declarations = queries.reduce((acc, {
    ...
    for (const [hash, declaration] of Object.entries(declarations)) {
      if (!acc[hash]) {
        ...
        acc[`${hash}_${queryKey}`] = t__namespace.tsModuleDeclaration(t__namespace.identifier('Sanity'), t__namespace.tsModuleDeclaration(t__namespace.identifier(workspaceIdentifier), t__namespace.tsModuleDeclaration(t__namespace.identifier('Query'), t__namespace.tsModuleBlock([t__namespace.tsTypeAliasDeclaration(t__namespace.identifier(queryKey), undefined, substitutions[structure.hash])]))));
      }
    }
    return acc;
  }, {});
  ...
  return {
    declarations: {
      ...declarations,
      _ClientConfig: t__namespace.tsModuleDeclaration(t__namespace.identifier('Sanity'), t__namespace.tsModuleDeclaration(t__namespace.identifier(workspaceIdentifier), t__namespace.tsModuleDeclaration(t__namespace.identifier('Client'), t__namespace.tsModuleBlock([t__namespace.tsTypeAliasDeclaration(t__namespace.identifier('Config'), undefined, t__namespace.tsTypeLiteral(Object.entries(queryKeys).map(([queryKey, hash]) => {
        return t__namespace.tsPropertySignature(t__namespace.identifier(queryKey), t__namespace.tsTypeAnnotation(substitutions[`${hash}_${queryKey}`] || substitutions[hash]));
      })))]))))
    },
    substitutions
  };
...
function generateSchemaTypes({
  ...
  const topLevelTypes = topLevelSchemaNodes.map(node => {
    ...
    return {
      ...
      declarations: {
        ...declarations,
        [structure.hash]: t__namespace.tsModuleDeclaration(t__namespace.identifier('Sanity'), t__namespace.tsModuleDeclaration(t__namespace.identifier(workspaceIdentifier), t__namespace.tsModuleDeclaration(t__namespace.identifier('Schema'), t__namespace.tsModuleBlock([t__namespace.tsTypeAliasDeclaration(t__namespace.identifier(identifier), undefined, tsType)]))))
      },
      substitutions: {
        ...substitutions,
        [structure.hash]: t__namespace.tsTypeReference(t__namespace.tsQualifiedName(t__namespace.tsQualifiedName(t__namespace.tsQualifiedName(t__namespace.identifier('Sanity'), t__namespace.identifier(workspaceIdentifier)), t__namespace.identifier('Schema')), t__namespace.identifier(identifier)))
      }
    };
  });
...
exports.ResolveExpressionError = ResolveExpressionError;
exports.SchemaParseError = SchemaParseError;
exports.defaultStructures = defaultStructures;
...
//# sourceMappingURL=index.standalone.cjs.js.map
