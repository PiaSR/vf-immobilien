...
async function resolveIdentifier({
  ...
  if (binding) {
    ...
    if (node.type !== 'ImportDefaultSpecifier' && node.type !== 'ImportSpecifier') {
      return {
        node,
        file,
        scope,
    ...
    const importDeclaration = (0, _boundedFind.boundedFind)(resolve => {
      (0, _core.traverse)(binding.scope.block, {
        ImportDeclaration(n) {
    ...
    const nextIdentifier = 'imported' in node ? 'value' in node.imported ? node.imported.value : node.imported.name : 'default';
  ...
  const matchingExportSpecifier = (0, _boundedFind.boundedFind)(resolve => {
    (0, _core.traverse)(scope.path.node, {
      // export { named } // from './re-export';
      ExportNamedDeclaration(n) {
        const matchingExportSpecifier = n.node.specifiers.find(specifier => {
          const exportedName = 'value' in specifier.exported ? specifier.exported.value : specifier.exported.name;
          return exportedName === identifierName;
        });
        if (matchingExportSpecifier?.type === 'ExportSpecifier') {
          resolve({
            node: matchingExportSpecifier,
            scope: n.scope
          });
        }
      }
  ...
  if (!matchingExportSpecifier && identifierName === 'default') {
    ...
    const exportDefaultDeclarationResult = (0, _boundedFind.boundedFind)(resolve => {
      (0, _core.traverse)(scope.block, {
        // export default someExpression;
    ...
    if (!exportDefaultDeclarationResult) {
      throw new _resolveExpressionError.ResolveExpressionError(`Could not find default export in: ${filename}`);
    }
    return {
  ...
  if (matchingExportSpecifier) {
    const {
      node
    } = matchingExportSpecifier;
    const exportNamedDeclaration = (0, _boundedFind.boundedFind)(resolve => {
      (0, _core.traverse)(matchingExportSpecifier.scope.block, {
        ExportNamedDeclaration(n) {
          if (n.node.specifiers.includes(node)) {
            resolve(n.node);
          }
    ...
    const exportAll = (0, _boundedFind.boundedFind)(resolve => {
      (0, _core.traverse)(scope.block, {
    ...
    if (exportAll) {
      ...
      throw new _resolveExpressionError.ResolveExpressionError(`Could not find identifier \`${identifierName}\` in file: ${filename}`);
    }
  }
}